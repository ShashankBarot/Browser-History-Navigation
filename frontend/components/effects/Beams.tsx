...
function extendMaterial(
  ...
  const baseDefines: Record<string, unknown> = (physical as unknown as { defines?: Record<string, unknown> }).defines ?? {};
  ...
  vec3 getNormal(vec3 pos) {
    ...
    if (uLightMode > 0.5) {
      ...
      gl_FragColor.rgb = mix(vec3(1.0), chroma, clamp(energy * 0.98, 0.0, 0.94));
        ...
        material: { fog: true },
        uniforms: {
          diffuse: { value: new THREE.Color(...hexToNormalizedRGB(beamColor)) },
          time: { value: 0 },
          roughness: 0.3,
          metalness: 0.3,
          uSpeed: { value: speed },
